...
import com.yyox.mvp.ui.holder.AddressOrderItemHolder;
...
public class AddressOrderAdapter extends DefaultAdapter<Address> {

    public static final int TYPE_FOOTER = 1;  //说明是带有Footer的
    public static final int TYPE_NORMAL = 2;  //说明是不带有header和footer的

    public AddressOrderAdapter(List<Address> infos) {
        super(infos);
    }
    ...
    public BaseHolder<Address> onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_address_list_order, parent, false);
        BaseHolder<Address> mHolder = new AddressOrderItemHolder(view);
    ...
    public void onBindViewHolder(BaseHolder<Address> holder, int position) {
        if(getItemViewType(position) == TYPE_NORMAL && holder instanceof AddressOrderItemHolder){
